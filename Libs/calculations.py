...
def SDCalculator(nn, rpeaks = None):
    '''
    nn is a numpy array of given intervals
    '''
    ...
    SD1 = 0.5 * np.sqrt(2) * np.std(vs1)
    SD2 = np.sqrt(2*np.std(vsx)**2 - 0.5*np.std(vs1)**2)
...
def find_maxima(xx, tolerance, edge_mode, progress_bar):
    ...
    if edge_mode == INCLUDE_EDGE:
        logger.debug("Edge mode is Include Edge")
    ...
    if edge_mode == CIRCULAR:
        logger.debug("Edge mode is Circular")
        count = 0
